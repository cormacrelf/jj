...
fn test_merge_views_bookmarks() {
    ...
    let mut tx1 = repo.start_transaction(&settings);
    let main_bookmark_local_tx1 = write_random_commit(tx1.mut_repo(), &settings);
    tx1.mut_repo().set_local_bookmark_target(
        "main",
        RefTarget::normal(main_bookmark_local_tx1.id().clone()),
    );
    let feature_bookmark_tx1 = write_random_commit(tx1.mut_repo(), &settings);
    tx1.mut_repo().set_local_bookmark_target(
        "feature",
    ...
    let mut tx2 = repo.start_transaction(&settings);
    let main_bookmark_local_tx2 = write_random_commit(tx2.mut_repo(), &settings);
    let main_bookmark_origin_tx2 = write_random_commit(tx2.mut_repo(), &settings);
    let main_bookmark_origin_tx2_remote_ref = RemoteRef {
        target: RefTarget::normal(main_bookmark_origin_tx2.id().clone()),
        state: RemoteRefState::Tracking,
    };
    tx2.mut_repo().set_local_bookmark_target(
        "main",
        RefTarget::normal(main_bookmark_local_tx2.id().clone()),
    );
    tx2.mut_repo().set_remote_bookmark(
        "main",
        "origin",
...
fn test_merge_views_git_refs() {
    ...
    let mut tx1 = repo.start_transaction(&settings);
    let main_bookmark_tx1 = write_random_commit(tx1.mut_repo(), &settings);
    ...
    let mut tx2 = repo.start_transaction(&settings);
    let main_bookmark_tx2 = write_random_commit(tx2.mut_repo(), &settings);
