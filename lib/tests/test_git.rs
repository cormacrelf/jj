...
fn test_import_refs_reimport_git_head_without_ref() {
    ...
    // Reimport HEAD, which doesn't abandon the old HEAD bookmark because jj thinks
    // it would be moved by `git checkout` command. This isn't always true
    // because the detached HEAD commit could be rewritten by e.g. `git commit
    // --amend` command, but it should be safer than abandoning old checkout
    // bookmark.
    git::import_head(tx.mut_repo()).unwrap();
    git::import_refs(tx.mut_repo(), &git_settings).unwrap();
    tx.mut_repo().rebase_descendants(&settings).unwrap();
...
fn test_import_refs_reimport_git_head_with_fixed_ref() {
    ...
    // Reimport HEAD, which shouldn't abandon the old HEAD bookmark.
    git::import_head(tx.mut_repo()).unwrap();
    git::import_refs(tx.mut_repo(), &git_settings).unwrap();
    tx.mut_repo().rebase_descendants(&settings).unwrap();
    assert!(tx.mut_repo().view().heads().contains(commit1.id()));
...
fn test_import_some_refs() {
    ...
    fn get_remote_bookmark(ref_name: &RefName) -> Option<&str> {
        match ref_name {
            RefName::RemoteBranch { bookmark, remote } if remote == "origin" => Some(bookmark),
            _ => None,
        }
    }
...
fn test_fetch_success() {
    ...
    assert_eq!(stats.default_bookmark, Some("main".to_string()));
    assert!(stats.import_stats.abandoned_commits.is_empty());
    let repo = tx.commit("test");
...
fn test_fetch_prune_deleted_ref() {
    ...
    .unwrap();
    // Test the setup
    assert!(tx.mut_repo().get_local_bookmark("main").is_present());
    assert!(tx
        .mut_repo()
    ...
    assert!(tx.mut_repo().get_local_bookmark("main").is_absent());
    assert!(tx
        .mut_repo()
        .get_remote_bookmark("main", "origin")
        .is_absent());
...
#[test]
fn test_fetch_no_default_bookmark() {
    let test_data = GitRepoData::create();
    let git_settings = GitSettings {
        auto_local_bookmark: true,
        ..Default::default()
