...
// TODO: Remove in jj 0.27+
pub fn cmd_branch(
    ui: &mut Ui,
    command: &CommandHelper,
    subcommand: &BookmarkCommand,
) -> Result<(), CommandError> {
    type WrappedCmd<'a> = dyn Fn(&mut Ui) -> Result<(), CommandError> + 'a;
    let cmd: Box<WrappedCmd> = match subcommand {
        BookmarkCommand::Create(args) => Box::new(|ui: &mut Ui| {
            renamed_cmd("branch create", "bookmark create", cmd_bookmark_create)(ui, command, args)
        }),
        BookmarkCommand::Delete(args) => Box::new(|ui| {
            renamed_cmd("branch delete", "bookmark delete", cmd_bookmark_delete)(ui, command, args)
        }),
        BookmarkCommand::Forget(args) => Box::new(|ui| {
            renamed_cmd("branch forget", "bookmark forget", cmd_bookmark_forget)(ui, command, args)
        }),
        BookmarkCommand::List(args) => Box::new(|ui| {
            renamed_cmd("branch list", "bookmark list", cmd_bookmark_list)(ui, command, args)
        }),
        BookmarkCommand::Move(args) => Box::new(|ui| {
            renamed_cmd("branch move", "bookmark move", cmd_bookmark_move)(ui, command, args)
        }),
        BookmarkCommand::Rename(args) => Box::new(|ui| {
            renamed_cmd("branch rename", "bookmark rename", cmd_bookmark_rename)(ui, command, args)
        }),
        BookmarkCommand::Set(args) => Box::new(|ui| {
            renamed_cmd("branch set", "bookmark set", cmd_bookmark_set)(ui, command, args)
        }),
        BookmarkCommand::Track(args) => Box::new(|ui| {
            renamed_cmd("branch track", "bookmark track", cmd_bookmark_track)(ui, command, args)
        }),
        BookmarkCommand::Untrack(args) => Box::new(|ui| {
            renamed_cmd("branch untrack", "bookmark untrack", cmd_bookmark_untrack)(
                ui, command, args,
            )
        }),
    };
